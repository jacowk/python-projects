...
class AccountAnalyzer:
    ...
    def __init__(self, date, account_type, input_filename):
        self.date = date
    ...
    def analyze(self):
        ...
        output_path = os.path.join(base_path, self.account_type, self.date)
        self.prepare_directory(output_path)

        input_copy_filename = "{}.csv".format(self.input_filename)
        input_copy_filename_path = os.path.join(base_path, output_path, input_copy_filename)

        copyfile("{}.csv".format(self.input_filename), input_copy_filename_path)

        transaction_filename = "{}TransactionHistory{}.json".format(self.account_type, self.date)
        transaction_path = os.path.join(base_path, output_path, transaction_filename)

        summary_filename = "{}TransactionSummary{}.json".format(self.account_type, self.date)
        summary_path = os.path.join(base_path, output_path, summary_filename)

        summary = {"date stamp": current_date_time, "period": self.date}

        # Read the CSV file
        csv_file = pd.read_csv("{}.csv".format(self.input_filename))
        ...
        for expense_transaction_type in self.expense_transaction_types:
        ...
        income_transactions_list = []
        for income_transaction_type in self.income_transaction_types:
            total = 0
            no_of_transactions = 0
    ...
    def set_expense_transaction_types(self, expense_transaction_types):
        self.expense_transaction_types = expense_transaction_types

    def set_income_transaction_types(self, income_transaction_types):
        self.income_transaction_types = income_transaction_types

    def set_prepare_directory(self, prepare_directory):
        self.prepare_directory = prepare_directory
